...
@section('title', 'Data Deteksi GPPH')
...
<div x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="space-y-1">
            ...
            <h2 class="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Deteksi GPPH</h2>
        </div>
        @can('create gpph')
    ...
    <div class="card-premium overflow-hidden bg-white shadow-xl shadow-slate-200/50 border-none">
        <div class="p-6 border-b border-slate-50 flex items-center gap-3">
            ...
            <h3 class="text-xs font-black text-slate-700 uppercase tracking-widest">DAFTAR PERTANYAAN DETEKSI GPPH</h3>
        </div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group/row">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                ...
                                @endcan
                                @can('delete gpph')
                                <form action="{{ route('qgpph.destroy', ['id' => $q->id]) }}" method="POST" class="inline">
                                    @csrf @method('DELETE')
    ...
    <div x-show="modalOpen" 
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg relative z-10 overflow-hidden border border-slate-100"
            ...
            <form :action="editMode ? '{{ route('qgpph.update', ['id' => ':id']) }}'.replace(':id', formData.id) : '{{ route('qgpph.store') }}'" 
                  method="POST">
                @csrf
                ...
                <div class="p-8 space-y-6">
                    <div class="space-y-2">
                        ...
                        <textarea class="w-full bg-slate-50 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-red-50 transition-all outline-none resize-none" 
                                  rows="4" name="question_text" x-model="formData.question_text" required placeholder="Masukkan pertanyaan deteksi GPPH..."></textarea>
                    </div>
                </div>
