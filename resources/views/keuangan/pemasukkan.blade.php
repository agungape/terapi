    ...
    <div class="card-premium overflow-hidden">
        <div class="p-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between bg-white gap-4">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                <form action="{{ route('keuangan.pemasukkan') }}" method="GET" class="flex w-full md:w-auto gap-2">
                    <div class="w-full md:w-64 flex-1">
                        <input type="search" name="search" value="{{ request('search') }}" placeholder="Cari transaksi..." class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold focus:ring-2 focus:ring-emerald-50 focus:border-emerald-200 transition-all outline-none text-slate-600">
                    </div>
                    <button type="submit" class="shrink-0 bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-xl text-xs font-bold shadow-md transition-colors flex items-center justify-center gap-2">
                        <span class="hidden sm:inline">Cari</span>
                        <i data-lucide="search" class="w-3.5 h-3.5 sm:hidden"></i>
                    </button>
                </form>
                <span class="hidden lg:inline-block px-3 py-1 bg-slate-50 text-slate-400 border border-slate-100 rounded-lg text-[9px] font-black uppercase tracking-widest italic shrink-0">Real-time Sync</span>
            </div>
        </div>
