        ...
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <div class="image">
                @php
                    ...
                    $user = $auth::user();
                    $roles = $user->getRoleNames(); // Pastikan menggunakan Spatie Permission

                    // Default avatar path
                    $defaultAvatar = asset('assets/images/faces-clipart/pic-2.png');
                    $avatarPath = $defaultAvatar;

                    // Cek role terapis
                    if ($roles->contains('terapis')) {
                        $terapis = $user->terapis ?? Terapis::where('nama', $user->name)->first();

                        if ($terapis && $terapis->foto) {
                            $avatarPath = asset('storage/terapis/' . $terapis->foto);
                        }
                    }
                @endphp

                <img src="{{ $avatarPath }}" class="img-circle elevation-2" alt="{{ $user->name }}">
            </div>
            <div class="info">
                <a href="#" class="d-block">{{ $user->name }}</a>
                <span class="text-muted text-sm">{{ $user->getRoleNames()->first() }}</span>
            </div>
        </div>
    </div>
