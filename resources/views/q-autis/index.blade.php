...
@section('title', 'Data Deteksi Autis')
...
<div x-data="{ 
    ...
    formData: {
        id: '',
        question_text: '',
        no_urut: '{{ $autis->count() + 1 }}'
    },
    openCreateModal() {
        this.editMode = false;
        this.formData = { id: '', question_text: '', no_urut: '{{ $autis->count() + 1 }}' };
        this.modalOpen = true;
    },
    openEditModal(item) {
        this.editMode = true;
    ...
    <div class="card-premium overflow-hidden bg-white shadow-xl shadow-slate-200/50 border-none">
        <div class="p-6 border-b border-slate-50 flex items-center gap-3">
            ...
            <h3 class="text-xs font-black text-slate-700 uppercase tracking-widest">DAFTAR PERTANYAAN DETEKSI AUTIS</h3>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead class="bg-slate-50 border-b border-slate-100">
                    <tr>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest w-16 text-center">Urutan</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group/row">
                        <td class="px-6 py-4 text-xs font-black text-slate-500 text-center">#{{ $q->no_urut }}</td>
                            ...
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                ...
                                <form action="{{ route('qautis.destroy', ['id' => $q->id]) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" class="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm border border-red-100 btn-hapus"
                                            data-name="Pertanyaan urutan #{{ $q->no_urut }}">
    ...
    <div x-show="modalOpen" 
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg relative z-10 overflow-hidden border border-slate-100"
            ...
            <form :action="editMode ? '{{ route('qautis.update', ['id' => ':id']) }}'.replace(':id', formData.id) : '{{ route('qautis.store') }}'" 
                  method="POST">
                @csrf
                ...
                <div class="p-8 space-y-6">
                    <div class="space-y-2">
                        ...
                        <textarea class="w-full bg-slate-50 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-red-50 transition-all outline-none resize-none" 
                                  rows="3" name="question_text" x-model="formData.question_text" required placeholder="Masukkan pertanyaan autis..."></textarea>
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block">No. Urut Tampil</label>
                        <select name="no_urut" x-model="formData.no_urut" required 
                            ...
                            @php $jumlahPertanyaan = $autis->count(); @endphp
                            @for ($i = 1; $i <= $jumlahPertanyaan + 1; $i++)
                                <option value="{{ $i }}">Posisi ke-{{ $i }}</option>
                            @endfor
                        </select>
                    </div>
                </div>
