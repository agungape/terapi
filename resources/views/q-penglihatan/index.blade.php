...
@section('title', 'Data Deteksi Penglihatan')
...
<div x-data="{ 
    ...
    openCreateModal() {
        this.editMode = false;
        this.formData = { id: '', interpretasi: 'normal', question_text: '' };
        this.modalOpen = true;
    },
    openEditModal(item) {
        this.editMode = true;
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="space-y-1">
            ...
            <h2 class="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Deteksi Penglihatan</h2>
        </div>
        @can('create penglihatan')
    ...
    <div class="card-premium overflow-hidden bg-white shadow-xl shadow-slate-200/50 border-none">
        <div class="p-6 border-b border-slate-50 flex items-center gap-3">
            ...
            <h3 class="text-xs font-black text-slate-700 uppercase tracking-widest">DAFTAR DETEKSI PENGLIHATAN</h3>
        </div>
            ...
            <table class="w-full text-left">
                <thead class="bg-slate-50 border-b border-slate-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Kelompok Interpretasi</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center w-40">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @forelse ($penglihatan as $p)
                    <tr class="hover:bg-slate-50/50 transition-colors group/row">
                        <td class="px-6 py-4 text-xs font-bold text-slate-400 text-center">{{ $penglihatan->firstItem() + $loop->iteration - 1 }}</td>
                        <td class="px-6 py-4 text-sm font-bold text-slate-700 leading-relaxed">{{ $p->question_text }}</td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase tracking-widest border border-blue-100">
                                {{ str_replace('_', ' ', $p->interpretasi) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                @can('update penglihatan')
                                <button @click="openEditModal({ id: '{{ $p->id }}', interpretasi: '{{ $p->interpretasi }}', question_text: '{{ addslashes($p->question_text) }}' })"
                                ...
                                @endcan
                                @can('delete penglihatan')
                                <form action="{{ route('qpenglihatan.destroy', ['id' => $p->id]) }}" method="POST" class="inline">
                                    @csrf @method('DELETE')
    ...
    <div x-show="modalOpen" 
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg relative z-10 overflow-hidden border border-slate-100"
            ...
            <form :action="editMode ? '{{ route('qpenglihatan.update', ['id' => ':id']) }}'.replace(':id', formData.id) : '{{ route('qpenglihatan.store') }}'" 
                  method="POST">
                @csrf
                ...
                <div class="p-8 space-y-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Kelompok Interpretasi Usia</label>
                        <select class="w-full bg-slate-50 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-red-50 transition-all outline-none" 
                                name="interpretasi" x-model="formData.interpretasi">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea class="w-full bg-slate-50 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold focus:ring-4 focus:ring-red-50 transition-all outline-none resize-none" 
                                  rows="3" name="question_text" x-model="formData.question_text" required placeholder="Masukkan pertanyaan deteksi penglihatan..."></textarea>
                    </div>
                </div>
