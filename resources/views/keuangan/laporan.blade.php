        ...
        <form action="{{ route('keuangan.laporan') }}" method="GET" class="flex flex-col lg:flex-row items-end gap-6">
            <div class="w-full lg:w-1/3 space-y-2">
                ...
                <div class="relative group">
                    <i data-lucide="search" class="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300 group-focus-within:text-red-500 transition-colors"></i>
                    <input type="text" id="reservation" name="date_range" 
                           class="w-full bg-slate-50 border-slate-100 rounded-2xl pl-12 pr-6 py-4 text-sm font-bold focus:ring-4 focus:ring-red-50 transition-all outline-none" placeholder="Pilih Tanggal...">
                </div>
            </div>

            <div class="flex items-center gap-3 w-full lg:w-auto">
                <button type="submit" class="flex-1 lg:flex-none bg-slate-900 hover:bg-slate-800 text-white py-4 px-8 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all">
                    <i data-lucide="filter" class="w-4 h-4"></i> Filter Data
                </button>
                
                <a href="#" id="export-pdf" class="flex-1 lg:flex-none bg-red-500 hover:bg-red-600 text-white py-4 px-8 rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg shadow-red-100">
    ...
    <div class="card-premium overflow-hidden bg-white">
        <div class="p-6 border-b border-slate-50 flex items-center justify-between">
            <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                <i data-lucide="database" class="w-4 h-4 text-red-500"></i> HASIL AUDIT TRANSAKSI
            </h3>
        </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-6 py-5">
                            <span class="text-xs font-bold text-slate-500">{{ $report->tanggal }}</span>
                        </td>
                        <td class="px-6 py-5">
                        ...
                        <td class="px-6 py-5">
                            <p class="text-xs font-bold text-slate-700 tracking-tight">{{ $report->deskripsi }}</p>
                        </td>
                    ...
                    @empty
                    <tr><td colspan="5" class="px-6 py-24 text-center text-slate-300 italic font-bold">Tidak ada laporan untuk periode ini</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
