...
@section('title', 'E-Book Rekam Medis Anak (Gabungan)')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
            ...
            <i data-lucide="chevron-right" class="w-3 h-3"></i>
            <span class="text-slate-600">Detail E-Book (Gabungan)</span>
        </div>
            ...
            <div class="card-premium overflow-hidden border-none shadow-2xl shadow-slate-200/50" x-data="{ tab: 'riwayat' }">
                ...
                <div class="flex p-3 bg-slate-900 border-b border-slate-800 gap-2">
                    <button @click="tab = 'riwayat'"
                        ...
                        <i data-lucide="book" class="w-4 h-4"></i> Riwayat Gabungan
                    </button>
                        ...
                        <div class="group">
                                ...
                                <div class="flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-2">
                                            ...
                                            <span class="text-red-500 font-black">{{ $r->terapis->nama }} {{ $r->terapis_id_pendamping ? '& '.$r->terapisPendamping->nama : '' }}</span>
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="px-8 pb-12">
                                <div class="bg-slate-50/80 rounded-[2rem] border border-slate-100 p-6 md:p-10 space-y-8">
                                    @php
                                        $pemsPerilaku = $r->pemeriksaanGabungans->where('jenis_form', 'perilaku');
                                        $pemsFisio = $r->pemeriksaanGabungans->where('jenis_form', 'fisioterapi');
                                    @endphp
                                    
                                    @if($pemsPerilaku->count() > 0)
                                    <div class="mb-6">
                                        <h6 class="text-[10px] font-black text-white bg-red-500 px-4 py-2 rounded-lg inline-block uppercase mb-4 tracking-widest">A. Terapi Perilaku</h6>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pl-4">
                                            <div class="space-y-4">
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="space-y-3">
                                                    <div class="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                                                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">Evaluasi Sesi</p>
                                                        <p class="text-xs font-bold text-slate-600 leading-relaxed">{{ $pemsPerilaku->first()->keterangan ?? '-' }}</p>
                                                    </div>
                                                    <div class="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                                                        <p class="text-[8px] font-black text-red-500 uppercase tracking-widest mb-2 flex items-center gap-2">Catatan untuk Orang Tua</p>
                                                        <p class="text-xs font-bold text-slate-600 leading-relaxed">{{ $pemsPerilaku->first()->catatan_orang_tua ?? '-' }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    @if($pemsFisio->count() > 0)
                                    <div class="mt-8 pt-8 border-t border-slate-200">
                                        <h6 class="text-[10px] font-black text-white bg-blue-500 px-4 py-2 rounded-lg inline-block uppercase mb-4 tracking-widest">B. Fisioterapi</h6>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pl-4">
                                            <div class="space-y-4">
                                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-200 pb-2">Program & Aktivitas</p>
                                                <div class="space-y-3">
                                                    @foreach ($pemsFisio as $p)
                                                    <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm group/item hover:border-red-200 transition-all">
                                                        <div class="flex items-center gap-3 mb-2">
                                                            <div class="w-2 h-2 rounded-full bg-blue-400"></div>
                                                            <span class="text-xs font-bold text-slate-700">{{ $p->program->deskripsi }}</span>
                                                        </div>
                                                        <div class="pl-5">
                                                            <p class="text-[10px] font-medium text-slate-500 leading-relaxed">{{ $p->aktivitas_terapi }}</p>
                                                        </div>
                                                    </div>
                                                    @endforeach
                                                </div>
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="space-y-3">
                                                    <div class="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                                                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">Evaluasi Sesi</p>
                                                        <p class="text-xs font-bold text-slate-600 leading-relaxed">{{ $pemsFisio->first()->keterangan ?? '-' }}</p>
                                                    </div>
                                                    <div class="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                                                        <p class="text-[8px] font-black text-red-500 uppercase tracking-widest mb-2 flex items-center gap-2">Catatan untuk Orang Tua</p>
                                                        <p class="text-xs font-bold text-slate-600 leading-relaxed">{{ $pemsFisio->first()->catatan_orang_tua ?? '-' }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
