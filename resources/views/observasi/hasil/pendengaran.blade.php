...
@section('title', 'Hasil Observasi Pendengaran')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="space-y-1">
            <div class="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest">
                ...
                <i data-lucide="chevron-right" class="w-3 h-3"></i>
                <span class="text-slate-600">Hasil Pendengaran</span>
            </div>
            <h2 class="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Evaluasi Fungsi Pendengaran</h2>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="card-premium p-8 bg-white text-center flex flex-col items-center">
                ...
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic">Pemeriksaan Audiometri Awal</p>
                ...
                <div class="w-full grid grid-cols-2 gap-4 mt-8">
                    ...
                    <div class="p-4 bg-blue-50 rounded-2xl border border-blue-100">
                        <p class="text-[9px] font-black text-blue-400 uppercase mb-1">Kategori</p>
                        <p class="text-xs font-black text-blue-600 uppercase italic">Hearing</p>
                    </div>
                </div>
            </div>

            <div class="card-premium p-8 bg-slate-900 text-white relative overflow-hidden">
                <i data-lucide="ear" class="w-32 h-32 absolute -right-4 -bottom-4 text-white/5"></i>
                <h4 class="text-xs font-black uppercase tracking-widest mb-4 italic">Instruksi Medis</h4>
                <p class="text-[11px] text-slate-400 font-medium leading-relaxed">
                    Jika ditemukan lebih dari satu jawaban "TIDAK", pasien sangat disarankan untuk melakukan konsultasi lanjutan dengan Dokter Spesialis THT.
                </p>
            </div>
        </div>
        ...
        <div class="lg:col-span-8">
            <div class="card-premium bg-white overflow-hidden shadow-xl shadow-slate-200/50">
                <div class="px-8 py-6 bg-blue-600 text-white flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center">
                            <i data-lucide="bell-ring" class="w-5 h-5 text-white"></i>
                        </div>
                        <div>
                            <h4 class="text-xs font-black uppercase tracking-widest mb-0.5">HASIL INSTRUMEN PENYIMPANGAN PENDENGARAN</h4>
                            <p class="text-[9px] font-bold text-white/70 uppercase">Skoring Respon Akustik & Perilaku</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50 border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Pernyataan Evaluasi</th>
                                <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Respon</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            @foreach ($qrpendengaran as $index => $q)
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-8 py-5 text-[10px] font-black text-slate-300">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</td>
                                <td class="px-8 py-5">
                                    <p class="text-xs font-bold text-slate-600 leading-relaxed tracking-tight group-hover:text-slate-900 transition-colors uppercase italic">{{ $q->question->question_text ?? 'Pertanyaan tidak ditemukan / dihapus' }}</p>
                                </td>
                ...
                <div class="p-8 bg-slate-50 border-t border-slate-100 flex items-center gap-3">
                    <div class="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Data Hasil Terarsip secara Digital pada Bright Star Portal</span>
                </div>
            </div>
        </div>
    </div>
